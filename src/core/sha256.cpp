...
SHA256::result_type SHA256::operator()()
{
  ...
  SHA256::result_type result;
  std::copy(std::begin(m_hash), std::end(m_hash), std::begin(result));
  return result;
}


/// compute SHA256 of a memory block
SHA256::result_type SHA256::operator()(const void* data, std::size_t numBytes)
{
  reset();
  add(data, numBytes);
  return (*this)();
...
SHA256::result_type SHA256::operator()(const std::string& text)
{
  reset();
