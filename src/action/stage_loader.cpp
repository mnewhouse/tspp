...
ts::action::Stage_loader::Stage_loader()
{
}
...
void ts::action::Stage_loader::async_load(const action::Stage_data& stage_data)
{
    auto func = [=]()
    {
        return load_stage(stage_data);
    };
...
std::unique_ptr<ts::action::Stage> ts::action::Stage_loader::load_stage(const action::Stage_data& stage_data)
{
...
ts::utf8_string ts::action::to_string(Stage_loader_state state)
{
    switch (state)
    {
