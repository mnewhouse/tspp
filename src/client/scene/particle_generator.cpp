...
void ts::scene::Particle_generator::add_car(const world::Car* car)
{
    car_list_.emplace_back();
    car_list_.back().car = car;
}

void ts::scene::Particle_generator::remove_car(const world::Car* car)
{
...
void ts::scene::Particle_generator::render(sf::RenderTarget& render_target, sf::RenderStates render_states)
{
    particle_drawer_.render(render_target, render_states);
}

void ts::scene::Particle_generator::update(std::size_t ticks)
{
