...
namespace ts
{
    ...
    namespace game
    {
        ...
        {
        public:
            Local_loading_sequence(const cup::Cup* cup, const resources::Resource_store* resource_store);
            ~Local_loading_sequence() = default;

            void async_load(const resources::Track_handle& track);
            void poll();

            template <typename F>
            void set_stage_data_handler(F&& handler);
        ...
        private:
            ...
            std::function<void(const cup::Stage_data& stage_data)> stage_data_handler_;            
...
template <typename F>
void ts::game::Local_loading_sequence::set_stage_data_handler(F&& handler)
{
    stage_data_handler_ = handler;
}

template <typename F>
void ts::game::Local_loading_sequence::set_completion_handler(F&& handler)
{
    completion_handler_ = handler;
}

template <typename F>
void ts::game::Local_loading_sequence::set_state_change_handler(F&& handler)
{
